...
import { fmtDollarsFull, fmtPct, fmtNum, dscrColor, capRateColor } from '@/lib/utils'
          ...
          {!uw ? (
            ...
            <>
              ...
              <div className="card p-5 flex items-center justify-between gap-6">
                <div className="grid grid-cols-3 gap-8 flex-1 text-center">
                  <div>
                    <div className={`text-3xl font-bold fin-num ${dscrColor(uw.dscr)}`}>{fmtNum(uw.dscr)}</div>
                    <div className="text-[10px] text-slate-600 uppercase tracking-widest mt-1">DSCR</div>
                  </div>
                  <div>
                    <div className={`text-3xl font-bold fin-num ${capRateColor(uw.capRate)}`}>{fmtPct(uw.capRate)}</div>
                    <div className="text-[10px] text-slate-600 uppercase tracking-widest mt-1">Cap Rate</div>
                  </div>
                  <div>
                    <div className="text-3xl font-bold fin-num text-slate-200">{fmtPct(uw.cashOnCash)}</div>
...
EOFcat > ~/Downloads/ls-capital/src/app/\(app\)/underwrite/page.tsx << 'EOF'
'use client'
import { useState } from 'react'
import { computeUnderwriting, stressTest } from '@/lib/underwriting'
import { fmtDollarsFull, fmtPct, fmtNum, dscrColor, capRateColor } from '@/lib/utils'
